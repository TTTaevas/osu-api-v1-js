...
export namespace Mods {
  ...
  export function removeUnsupported(mods: Mods[]): Mods[] {
    if (
      mods.includes(Mods.NIGHTCORE) === true &&
      mods.includes(Mods.DOUBLETIME) === false
    ) {
   ...
   */
  export const adjustBeatmapStats = (
    beatmap: Beatmap,
    mods: Mods | Mods[],
  ): Beatmap => {
    ...
    if (mods.includes(Mods.DOUBLETIME) || mods.includes(Mods.NIGHTCORE)) {
      ...
      beatmap.diff_approach =
        (1950 - convertARtoMS(beatmap.diff_approach) / 1.5) / 150;
