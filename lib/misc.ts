...
export const getURL = {
   ...
   */
  beatmapCoverImage: (
    beatmap: { beatmapset_id: Beatmap["beatmapset_id"] } | Beatmap,
  ...
  toOpen: {
     ...
     */
    match: (id: number, password?: string) =>
      `osu://mp/${id}${password ? "/" + password : ""}`,
    /**
    ...
    channel: (channel: string) =>
      `osu://chan/${channel.startsWith("#") ? "" : "#"}${channel}`,
    /**
     ...
     */
    beatmap: (beatmap: { beatmap_id: Beatmap["beatmap_id"] } | Beatmap) =>
      `osu://b/${beatmap.beatmap_id}`,
    /**
     ...
     */
    spectateUser: (user: { user_id: User["user_id"] } | User) =>
      `osu://spectate/${user}`,
  },
};
