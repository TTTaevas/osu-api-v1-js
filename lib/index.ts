...
import {
  Score,
  ScoreWithBeatmapid,
  ScoreWithBeatmapidReplayavailablePp,
...
function correctType(x: any, force_string?: boolean): any {
  ...
  const bools = [
    ...
    "storyboard", // Beatmap
    "video", // Beatmap
    "download_unavailable", // Beatmap
    "audio_unavailable", // Beatmap
  ];
...
export class API {
  ...
  private async request(
    ...
    const response = await fetch(
      `${this.server}/${endpoint}?k=${this.key}&${parameters}`,
      {
        method: "get",
        headers: {
          ...
          "User-Agent":
            "osu-api-v1-js (https://github.com/TTTaevas/osu-api-v1-js)",
        },
      },
    ).catch((error) => {
    ...
    if (!response || !response.ok) {
      if (response) {
        ...
        if (response.status === 401) {
          this.log(
            true,
            "Server responded with status code 401, are you sure you're using a valid API key?",
          );
        } else if (response.status === 429) {
          this.log(
            true,
            "Server responded with status code 429, you're sending too many requests at once and are getting rate-limited!",
          );
          to_retry = true;
        } else {
      ...
      if (to_retry === true && number_try < max_tries) {
        this.log(
          true,
          "Will request again in a few instants...",
          `(Try #${number_try})`,
        );
    ...
    this.log(false, response.statusText, response.status, {
      endpoint,
      parameters,
    });
      ...
      if ("error" in json) {
        let err =
          typeof json.error === "string"
            ? json.error
            : "A strange error was returned by the server...";
        throw new APIError(err, this.server, endpoint, parameters);
      }
  ...
  async getUser(
    ...
    const lookup =
      user.user_id !== undefined
        ? `u=${user.user_id}&type=id`
        : `u=${user.username}&type=string`;
  ...
  async getUserBestScores(
    ...
    const scores = await this.request(
      "get_user_best",
      `${lookup}&m=${gamemode}&limit=${limit}`,
    );
    scores.forEach(
      (s: any) => (s.enabled_mods = Mods.bitsToArray(s.enabled_mods)),
    );
    return scores;
  }
  ...
  async getBeatmaps(
    limit: number,
    gamemode: { gamemode: Gamemodes | "all"; allow_converts?: boolean },
    beatmap?:
      | {
  ...
  async getBeatmapScores(
    ...
    user?: { user_id?: User["user_id"]; username?: User["username"] } | User,
    mods: Mods[] = [],
  ): Promise<ScoreWithReplayavailablePp[]> {
    ...
    const scores = await this.request(
      "get_scores",
      `b=${beatmap.beatmap_id}&m=${gamemode}${mods_bits !== null ? "&mods=" + mods_bits : ""}${user_lookup}&limit=${limit}`,
    );
    scores.forEach(
      (s: any) => (s.enabled_mods = Mods.bitsToArray(s.enabled_mods)),
    );
