...
if (key === undefined) {
  throw new Error(
    "❌ The API key has not been defined in the environment variables! (name of the variable is `KEY`)",
  );
}
...
function roundTo(n: number, digits: number) {
  ...
  if (negative) {
    x = (Number(x) * -1).toFixed(digits);
  }
  return Number(x);
}

const checkBeatmapWithMods = (
  b: osu.Beatmap,
  mods: osu.Mods[],
  expected: object,
) => {
  ...
  if (JSON.stringify(stats) !== JSON.stringify(expected)) {
    ...
    console.error(
      `❌ The beatmap's stats with the mods ${mods.map((m) => osu.Mods[m])} are not what they should be!\n`,
    );
    return false;
  } else if (bm.difficultyrating <= 0) {
    console.error(
      `❌ The beatmap's star rating with the mods ${mods.map((m) => osu.Mods[m])} is 0* or below! (it's ${bm.difficultyrating})`,
    );
    return false;
  }
  return true;
...
const getUserRecentScores = async (): Promise<boolean> => {
  try {
    const scores = await api.getUserRecentScores(2, osu.Gamemodes.OSU, {
      user_id: 5795337,
    });
...
const getUserBestScores = async (): Promise<boolean> => {
  try {
    const scores = await api.getUserBestScores(2, osu.Gamemodes.OSU, {
      user_id: 2,
    });
    expect(scores).to.have.lengthOf(2);
...
const getBeatmapScores = async (): Promise<boolean> => {
  try {
    const scores = await api.getBeatmapScores(
      5,
      osu.Gamemodes.OSU,
      { beatmap_id: 129891 },
      { user_id: 124493 },
      [osu.Mods.HIDDEN, osu.Mods.HARDROCK],
    );
...
const getReplay = async (): Promise<boolean> => {
  try {
    const replay = await api.getReplay(osu.Gamemodes.OSU, {
      score_id: 2177560145,
    });
...
const testBeatmapWithMods = async (): Promise<boolean> => {
  ...
  try {
    const beatmap = await api.getBeatmap({ beatmap_id: 2592029 }, [
      osu.Mods.NOFAIL,
    ]);
    ...
    for (let i = 0; i < dtnc.length; i++) {
      expect(
        checkBeatmapWithMods(beatmap, [dtnc[i]], {
          bpm: 294,
          cs: 3,
          ar: 9,
          od: 8.44,
      ...
      expect(
        checkBeatmapWithMods(beatmap, [dtnc[i], osu.Mods.EASY], {
          bpm: 294,
          cs: 1.5,
          ar: 6.87,
          od: 6.44,
      ...
      expect(
        checkBeatmapWithMods(beatmap, [dtnc[i], osu.Mods.HARDROCK], {
          bpm: 294,
          cs: 3.9,
          ar: 10.87,
          od: 10.04,
    ...
    expect(
      checkBeatmapWithMods(beatmap, [osu.Mods.HALFTIME], {
        bpm: 147,
        cs: 3,
        ar: 5,
        od: 3.56,
    ...
    expect(
      checkBeatmapWithMods(beatmap, [osu.Mods.EASY], {
        bpm: 196,
        cs: 1.5,
        ar: 3.5,
        od: 3,
    ...
    expect(
      checkBeatmapWithMods(beatmap, [osu.Mods.HARDROCK], {
        bpm: 196,
        cs: 3.9,
        ar: 9.8,
        od: 8.4,
    ...
    expect(
      checkBeatmapWithMods(beatmap, [osu.Mods.HALFTIME, osu.Mods.EASY], {
        bpm: 147,
        cs: 1.5,
        ar: -0.33,
        od: -0.44,
    ...
    expect(
      checkBeatmapWithMods(beatmap, [osu.Mods.HALFTIME, osu.Mods.HARDROCK], {
        bpm: 147,
        cs: 3.9,
        ar: 8.73,
        od: 6.76,
